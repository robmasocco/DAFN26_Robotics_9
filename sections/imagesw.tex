...
\begin{frame}{ROS tools for image processing}{A quick overview}
  \justifying
	\textbg{ROS 2} provides a wide range of \textbg{tools} to acquire, process, and transmit images.\\
  The most important ones are
	\begin{itemize}
		...
		\item \href{https://wiki.ros.org/image_transport}{\color{blue}\underline{\texttt{image\_transport}}}: a ROS package to \textbg{transmit images and metadata} over the RMW layer efficiently;
...
# This may be "RGB", "BGR", "RGBA", "BGRA", "YUV", "Bayer" ...
string encoding
...
\end{lstlisting}
	\end{columns}
\end{frame}
\begin{frame}[fragile]{ROS tools for image processing}{The CameraInfo message}
	\begin{columns}
		\column{.9\textwidth}
		\begin{lstlisting}[language=ros2msg, caption=Definition of the \texttt{sensor\_msgs/msg/CameraInfo} message; go have a look at it.]
std_msgs/Header header
...
float64[9]  k # Intrinsic raw camera matrix
float64[9]  r # Rectification matrix (rotated stereo cameras)
...
\begin{frame}{ROS tools for image processing}{Sending images over the DDS}
  \justifying
	...
	\end{enumerate}
	\texttt{image\_transport} is a ROS 2 library that provides \textbg{wrappers} for \texttt{Image+CameraInfo} \textbg{topic publishers} and \textbg{subscribers}, allowing to transmit images using different \textbg{transports}:
	\begin{itemize}
...
\begin{frame}{ROS tools for image processing}{A note on topics and their statistics}
    ...
    \justifying
    When measuring image topics publishing rates with \texttt{ros2 topic hz}, especially \textbr{over a lossy network}, you \textbr{may} get \textbr{high variance} or straight out \textbr{wrong results}.\\
    This is due to:
    \begin{itemize}
      \item the \textbr{Python DDS implementation};
      \item the \textbr{DDS struggling with large messages};
      \item the \texttt{hz} Python tool \textbr{buffering} messages into a window-based filter.
    \end{itemize}
	\end{alertblock}
\end{frame}
