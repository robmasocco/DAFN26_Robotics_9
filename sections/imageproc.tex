...
% --- Lens distortion ---
\begin{frame}{Lens distortion}{Camera calibration and rectification}
	\begin{columns}
		\column{.5\textwidth}
    \justifying
		\textbg{Pinhole cameras} generally introduce \textbg{radial distortion} in the images they produce, \emph{i.e.}, \textbg{straight lines appear curved}. This is due to how the light enters the camera through the lens.\\
		\bigskip
		A \textbg{camera calibration} procedure is needed to estimate the \textbg{intrinsic camera parameters} of the \textbg{distortion model}, so that a \textbg{rectification map} can then be applied to each frame.
...
\begin{frame}{Lens distortion}{Camera calibration and rectification}
    ...
    \justifying
		...
		\bigskip
		An \textbg{estimation model} can then be used to estimate the parameters of the distortion model, which can then be used to build the rectification map.
...
% --- Event cameras ---
\begin{frame}{Event cameras}{A new frontier in computer vision for robotics}
	\begin{figure}
		\centering
...
\begin{frame}{Event cameras}{A new frontier in computer vision for robotics}
    ...
    \justifying
		\only<1>{
			\nolistindent Also called \textbg{neuromorphic cameras}, they are a new type of visual sensor that provides \textbg{events} instead of \textbg{frames}, and are \textbg{complementary} to traditional cameras.
		}
		\only<2>{
			\hspace{-8pt}They are characterized by
			\begin{itemize}
				\item \textbg{no motion blur};
				\item \textbg{high dynamic range};
				\item \textbg{very low latency} (microseconds);
				\item \textbg{low power consumption} (milliwatts).
			\end{itemize}
		}
    \only<3>{
      \hspace{-12pt}They are very recent (circa 2008), and are still in the \textbg{research phase}. They are being evaluated in \textbg{high-speed} and \textbg{high-dynamic range} applications, such as
      \begin{itemize}
        \item \textbg{autonomous driving};
        \item \textbg{localization and mapping};
        \item \textbg{object tracking};
        \item \textbg{industrial automation, inspection, and visual servoing};
        \item \textbg{artificial intelligence}.
      \end{itemize}
    }
		...
		\begin{figure}
			\centering
			\includegraphics[width=.9\textwidth]{event_camera.png}
			\caption{Event camera from SynSense.}
			\label{fig:eventcamera}
		\end{figure}
	\end{columns}
\end{frame}
\begin{frame}{Event cameras}{A new frontier in computer vision for robotics}
